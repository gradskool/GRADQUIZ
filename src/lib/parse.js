...
export function parseQuestions(text) {
  ...
  const close = () => {
    ...
    if (!cur.body) fail('the question text is missing.')
      ...
      else questions.push({ kind: 'tita', body: cur.body, options: [], correct_index: null, accepted: cur.accepted })
    ...
    else questions.push({ kind: 'mcq', body: cur.body, options: cur.options, correct_index: cur.answer, accepted: null })
    cur = null
  }
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (letter && cur) {
      if (cur.options.length === 0) cur.problem = 'has an answer letter but no options. For a type-in question write the value, like "Ans: 42".'
      else cur.answer = letter[1].toUpperCase().charCodeAt(0) - 65
      close()
      continue
    }
    ...
    if (typed && cur) {
      if (cur.options.length > 0) cur.problem = 'has options, so the answer must be a letter like "Ans: B".'
      else cur.accepted = [...new Map(typed[1].split('|').map((x) => x.trim()).filter(Boolean).map((x) => [x.toLowerCase(), x])).values()].slice(0, 10)
      close()
      continue
    }
    ...
    if (!cur || (startsQ && cur.options.length > 0)) {
      if (cur) close()
      n += 1
      cur = { body: line.replace(QPREFIX, '').trim(), options: [], answer: null, accepted: null, problem: null }
